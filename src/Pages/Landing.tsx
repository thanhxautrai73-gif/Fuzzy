...
const Landing = () => {
  ...
  useEffect(() => {
    apiRequest('/categories')
      .then((data) => {
        ...
        const sofa = data.find((c: Category) => c.slug === 'sofa');
        if (sofa) {
  ...
  // Khởi tạo Swiper cho Categories và Offer Zone khi có dữ liệu
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if ((window as any).Swiper) {
        try {
          ...
          // Khởi tạo swiper cho offer zone
          if (products.length > 0 && !selectedCategory && !searchTerm) {
    ...
    return () => {
      if (categoriesSwiper && categoriesSwiper.destroy) categoriesSwiper.destroy(true, true);
      if (offerSwiper && offerSwiper.destroy) offerSwiper.destroy(true, true);
      clearTimeout(timer);
    };
  }, [categories, products, selectedCategory, searchTerm]);

  // Phân nhóm sản phẩm cho trang chủ (giống 100% template HTML gốc)
    ...
    <div className="text-start">
        ...
        <div className="offcanvas-body">
          <div className="sidebar-content">
            <ul className="link-section">
              ...
              <li>
                <Link to="/orders" className="pages">
              ...
              <li>
                <Link to="/admin" className="pages">
                  ...
                  <i className="iconsax text-danger" data-icon="arrow-right" />
                </Link>
              </li>
              <li>
                {user ? (
                  <a href="#!" onClick={() => { logout(); navigate('/login'); }} className="pages text-danger">
                    <h4>Đăng xuất</h4>
                  </a>
                ) : (
                  <Link to="/login" className="pages text-primary">
                    <h4>Đăng nhập</h4>
                  </Link>
                )}
              </li>
      ...
      <header className="section-t-space">
        <div className="custom-container">
          <div className="header">
            ...
            <Link to="/orders" className="notification">
          ...
          <form className="theme-form search-head" onSubmit={handleSearchSubmit}>
            <div className="form-group">
              <div className="form-input">
                ...
                />
                <i className="iconsax search-icon" data-icon="search-normal-2" onClick={() => fetchProducts(1, false)} style={{cursor: 'pointer'}} />
              </div>
      ...
      {loading ? (
                    ...
                    <div className="product-box">
                      <div className="product-box-img position-relative" style={{ aspectRatio: '1/1', overflow: 'hidden', borderRadius: '12px' }}>
                        ...
                        <div className="cart-box">
                          <Link to={`/product/${prod.id}`} className="cart-bag">
                            <i className="iconsax bag" data-icon="basket-2" />
                          </Link>
                        </div>
                      </div>
                      {/* Thêm like-btn chuẩn cấu trúc thiết kế mẫu */}
                      <div className={`like-btn animate ${prod.name === 'Buddy Chair' || prod.name === 'Bubble Swing Chair' ? 'active' : ''} inactive`}>
        ...
        <>
          ...
          <section className="section-t-space section-b-space pt-0">
            <div className="custom-container">
              <div className="title d-flex justify-content-between align-items-center mb-3">
